...
class QFNWizard(FootprintWizardBase.FootprintWizard):
    ...
    def GenerateParameterList(self):
        self.AddParam("Pads", "pins in X direction", self.uInteger, 25, min_value=0)
        self.AddParam("Pads", "pins in Y direction", self.uInteger, 25, min_value=1)
        ...
        self.AddParam("Pads", "v pitch", self.uMM, 14)
        self.AddParam("Pads", "h pitch", self.uMM, 14)
    ...
    def GetValue(self):
        ...
        return "QFN-{n}_{ep}{x:g}x{y:g}mm_Pitch{p:g}mm".format(
                n = 2*(self.pads["pins in X direction"] + self.pads["pins in Y direction"]),
    ...
    def BuildThisFootprint(self):
        ...
        v_pitch = self.pads["v pitch"]
        h_pitch = self.pads["h pitch"]

        pads_per_row_x = int(self.pads["pins in X direction"])
        pads_per_row_y = int(self.pads["pins in Y direction"])
        ...
        if self.epad['epad'] == True:
            ...
            epad_num = 2 * (self.pads["pins in X direction"] + self.pads["pins in Y direction"]) + 1
        ...
        clip_x = (pads_per_row_x + 1) * pad_pitch / 2
        clip_y = (pads_per_row_y + 1) * pad_pitch / 2
        ...
        if pads_per_row_x != 0:
            ...
            # Add pin-1 indication as per IPC-7351C
            self.draw.Line(w/2+offset, -h/2-offset, -w/2-pad_length/2, -h/2-offset)

            self.draw.Line(w/2+offset, h/2-offset, -w/2-offset, h/2-offset)
        ...
        sizey = (lim_y + cmargin) * 2 + (pad_length if pads_per_row_x != 0 else 0)
